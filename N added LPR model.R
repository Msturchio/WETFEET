...
home<-"/Volumes/The Hive/WETFEED Analysis" # home
work<-"/Volumes/The Hive/WETFEED Analysis" # work
raw<-"/Volumes/The Hive/WETFEED Analysis"
hist<-"/Volumes/The Hive/WETFEED Analysis"
lins<-"/Volumes/The Hive/WETFEED Analysis"
deets<-"/Volumes/The Hive/WETFEED Analysis"

setwd(work)


df<-read.csv("completeLMA2.csv")
df<-subset(df, KEEP == "Y")
...
for (reps in unique(df$var)){
  ...
  temp.df$Narea<-unique(dat$Narea)
  temp.df$Nmass..g.N.kg.1.<-unique(dat$Nmass..g.N.kg.1.)
...
setwd(deets)
write.csv(output, "NaddedtoLPR.csv")
...
write.csv(new, "them intercepts january 2021.csv")
