...
library(sjPlot)
library(lattice);library(doBy); library(lubridate)
library(minpack.lm); library(plotrix); library(MASS);library(plotmath)
...
##################################
tiff(file = "Longterm Q10 Insitu 18.tiff", height = 6, width = 6, res = 1200, units = "in", compression = "zip+p")
par(mfrow = c(2,2), omi = c(0.5, 0.5, 0.25, 0.1), mar = c(1,1,1,0.5))
...
text(21, 2.2, expression(North~Site:~italic(R)[18]~'= 0.62±0.02'~","~italic(Q)[10]~'=2.01±0.09'),cex=0.65)
text(21, 2, expression(South~Site:~italic(R)[18]~'= 0.47±0.03'~","~italic(Q)[10]~'=3.12±0.24'),cex=0.65)
...
#North Site
K<-273.15; df$alpha<-df$AirTemp_C.mean+K; df$beta<-K+17
q10 <- function(x) {0.62385 * (2.0133) ^ (((x+273.15)-290.15)/10)}
points(q10(seq(16.5,28.5,1)) ~ seq(16.5,28.5,1), type = "l", lwd = 2,  col = "black")
...
text(21, 2.4, expression(italic(R)[18]~'= 0.86±0.03'~","~italic(Q)[10]~'=1.79±0.09'),cex=0.65)
...
K<-273.15; df$alpha<-df$AirTemp_C.mean+K; df$beta<-K+17
q10 <- function(x) {0.86083 * (1.79215) ^ (((x+273.15)-290.15)/10)}
points(q10(seq(16.5,28.5,1)) ~ seq(16.5,28.5,1), type = "l", lwd = 2, col = "grey60")
...
axis(1, at = seq(15,29,2), cex.axis = 1.2, labels = F)
axis(2, at = seq(0,3,0.5),las = 2, cex.axis = 1, labels = F)
...
text(21, 12.5, expression(North~Site:~italic(R)[18]~'= 2.35±0.13'~","~italic(Q)[10]~'=2.25±0.15'),cex=0.65)
text(21, 11.5, expression(South~Site:~italic(R)[18]~'= 1.54±0.22'~","~italic(Q)[10]~'=5.14±0.84'),cex=0.65)
#North Site
K<-273.15; df$alpha<-df$AirTemp_C.mean+K; df$beta<-K+17
q10 <- function(x) {2.365 * (2.2519) ^ (((x+273.15)-290.15)/10)}
points(q10(seq(16.5,28.5,1)) ~ seq(16.5,28.5,1), type = "l", lwd = 2, col = "black")


#South Site
K<-273.15; df$alpha<-df$AirTemp_C.mean+K; df$beta<-K+17
q10 <- function(x) {1.5410 * (5.1482) ^ (((x+273.15)-290.15)/10)}
points(q10(seq(18,28.5,1)) ~ seq(18,28.5,1), type = "l", lwd = 2, lty = 3, col = "black")
...
text(21, 13, expression(italic(R)[18]~'= 7.42±0.20'~","~italic(Q)[10]~'=1.34±0.05'),cex=0.65)
# text(23, 3, expression(italic(r)^2~'= 0.40'),cex=1)

K<-273.15; df$alpha<-df$AirTemp_C.mean+K; df$beta<-K+17
q10 <- function(x) {7.41902 * (1.34446) ^ (((x+273.15)-290.15)/10)}
points(q10(seq(16.5,28.5,1)) ~ seq(16.5,28.5,1), type = "l", lwd = 2, col = "grey60")
...
axis(1, at = seq(15,29,2), cex.axis = 1, labels =T)
axis(2, at = seq(0,16,4),las = 2, cex.axis = 1, labels = F)
